...
const DEFAULT_INNER_GRADIENT =
  "linear-gradient(165deg, rgba(24,24,32,0.98) 0%, rgba(15,15,20,1) 55%, rgba(99,102,241,0.06) 100%)";
...
const ProfileCardComponent = ({
  ...
  const shineStyle = useMemo(
    () => ({
      maskImage: showHoloPattern
        ? "var(--icon)"
        : "linear-gradient(to bottom, black 0%, black 28%, transparent 52%)",
      maskMode: showHoloPattern ? "luminance" : "alpha",
      maskRepeat: showHoloPattern ? "repeat" : "no-repeat",
      maskSize: showHoloPattern ? "150%" : "100% 100%",
      maskPosition: showHoloPattern
        ? "top calc(200% - (var(--background-y) * 5)) left calc(100% - var(--background-x))"
        : "center top",
      WebkitMaskImage: showHoloPattern
        ? "var(--icon)"
        : "linear-gradient(to bottom, black 0%, black 28%, transparent 52%)",
      filter: "brightness(0.9) contrast(1.05) saturate(0.35) opacity(0.18)",
      ...
      overflow: "hidden",
      zIndex: 3,
      backgroundSize: grainUrl
        ? "250% 250%, 100% 100%, 100% 100%, 220px 220px"
        : "250% 250%, 100% 100%, 100% 100%",
      backgroundPosition: grainUrl
        ? "var(--pointer-x) var(--pointer-y), center, center, calc(var(--pointer-x) * 0.01) calc(var(--pointer-y) * 0.01)"
        : "var(--pointer-x) var(--pointer-y), center, center",
      backgroundImage: `
      ...
      radial-gradient(
        ...
        hsla(228, 60%, 60%, 0.08) 12%,
        transparent 80%
      )${grainUrl ? `, url(${grainUrl})` : ""}
    `.replace(/\s+/g, " "),
      gridArea: "1 / -1",
