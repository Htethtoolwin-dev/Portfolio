...
function ProjectLinks({ project, variant = "overlay" }) {
  ...
  const overlayClass = "inline-flex min-h-10 items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition-transform active:scale-95 sm:px-4 sm:text-sm";
  const inlineClass = "inline-flex min-h-10 flex-1 items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition-colors active:scale-95 sm:text-sm";

  if (variant === "overlay") {
    return (
      <>
        {project.liveUrl && (
          <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className={`${overlayClass} bg-white text-gray-900 hover:scale-105`}>
            <ExternalLink size={16} /> Live Demo
          </a>
        )}
        {project.githubUrl && (
          <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className={`${overlayClass} border border-white/30 bg-white/10 text-white backdrop-blur hover:scale-105`}>
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {project.liveUrl && (
        <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className={`${inlineClass} bg-[var(--accent)] text-white hover:bg-[var(--accent-hover)]`}>
          <ExternalLink size={16} /> Live Demo
        </a>
      )}
      {project.githubUrl && (
        <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className={`${inlineClass} border border-[var(--border)] bg-[var(--bg-primary)] text-[var(--text-primary)] hover:border-[var(--accent)] hover:text-[var(--accent)]`}>
          <GitHubIcon size={16} /> Code
        </a>
      )}
    </div>
